...
async def create_vectorstore(
    ...
    if globals()["file_count"] != new_file_count:
        ...
        ids = [str(uuid.uuid5(uuid.NAMESPACE_DNS, str(chunk.metadata) + chunk.page_content)) for chunk in chunked_docs]
        ...
        print(f"Docs added. There are now {len(ids)} chunks from {new_file_count} documents in the store.")
